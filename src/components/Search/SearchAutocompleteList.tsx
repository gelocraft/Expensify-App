...
function SearchAutocompleteList({
    ...
    const autocompleteSuggestions = useMemo<AutocompleteItemData[]>(() => {
        ...
        const alreadyAutocompletedKeys = new Set(ranges
            .filter((range) => {
        ...
        switch (autocompleteKey) {
            case CONST.SEARCH.SYNTAX_FILTER_KEYS.TAG: {
                ...
                const filteredTags = autocompleteList
                    .filter(
                        (tag) => getCleanedTagName(tag).toLowerCase().includes(autocompleteValue.toLowerCase()) && !alreadyAutocompletedKeys.has(getCleanedTagName(tag).toLowerCase()),
                    )
                    .sort()
                    .slice(0, 10);
            ...
            case CONST.SEARCH.SYNTAX_FILTER_KEYS.EXPENSE_TYPE: {
                const filteredExpenseTypes = expenseTypes
                    .filter((expenseType) => expenseType.includes(autocompleteValue.toLowerCase()) && !alreadyAutocompletedKeys.has(expenseType))
                    .sort();
